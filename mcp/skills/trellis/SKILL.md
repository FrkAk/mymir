---
name: trellis
description: >
  Persistent context network for coding projects. Tracks tasks, dependencies, decisions, and implementation records across sessions.
  AUTO-INVOKE when the user:
  - Describes a new project or app idea
  - Asks what to work on next, what's left, or what's blocked
  - Reports progress, completion, or decisions on tasks
  - Asks about project structure, dependencies, or architecture
  - Says "continue", "resume", or wants to pick up previous work
  - Mentions decomposition, planning, or breaking down work
  - Wants to dispatch work to multiple agents
  DO NOT invoke for: general coding questions, debugging, file editing, or git operations.
---

# Trellis — Context Network for Coding Projects

Invokable as `/trellis`. You have access to 6 Trellis MCP tools (prefixed `trellis_`) for managing project context across sessions.

## First Use in Session

1. `trellis_project` with `action='list'` → see existing projects
2. `trellis_project` with `action='select'` → set current project (`create` auto-selects)
3. Then use other tools as needed

## Data Model

**Project** → **Tasks** (flat list with categories for drawer grouping)

Edges connect tasks: `depends_on` (source needs target done first), `relates_to` (informational link)

Tasks have: title, description, status, category, acceptanceCriteria, decisions, tags, files, implementationPlan, executionRecord

`category` determines drawer grouping (one per task, defined at project level). `tags` are freeform for filtering.

Task titles: verb+noun format (e.g., "Implement JWT auth", "Fix login redirect").

## Tools

| Tool | Actions/Types | Purpose |
|------|---------------|---------|
| `trellis_project` | list, create, select, update | Manage projects |
| `trellis_task` | create, update, delete, reorder | Manage tasks |
| `trellis_edge` | create, update, remove | Manage dependency edges |
| `trellis_query` | search, list, edges, overview | Find and browse data |
| `trellis_context` | summary, working, agent, planning | Task context at varying depth (see below) |
| `trellis_analyze` | ready, blocked, downstream, critical_path, plannable | Analyze dependency graph |

## Context Depths

| Depth | Use when | What it includes |
|-------|----------|-----------------|
| `summary` | Quick status check | JSON: status, edge counts |
| `working` | Refining, discussing, or reviewing a task | Criteria, decisions, edges (1-hop), siblings, conversation history |
| `agent` | Writing code for a task | Implementation plan, upstream execution records, files, "Done Means", downstream |
| `planning` | Writing an implementation plan | Project description, acceptance criteria, upstream execution records, downstream specs |

**Don't guess the depth.** When looking up a task, `trellis_query type='search'` returns a `state` field and `_hints` that tell you which depth to use. Follow the hints.

## Response Hints

Tool responses may include a `_hints` array with contextual guidance (missing fields, next steps, warnings). **Always read and follow these hints.**

## Agent Delegation

Two agents require dedicated delegation — **do not handle these yourself:**

| User intent | Agent | When |
|-------------|-------|------|
| New idea, "I want to build...", app concept | `trellis:brainstorm` | No project exists yet, or exploring a new idea |
| "Break this down", "decompose", "create tasks" | `trellis:decompose` | Project exists with description but few/no tasks |

All other project management (status, next task, refine, continue, mark done) is handled directly by this skill. The `trellis:manage` agent exists for explicit delegation only.

**If unsure:** `trellis_query type='overview'`. No tasks → decompose. Has tasks → use workflows below.

## Workflows

### "What should I work on?" / "What's next?"
1. `trellis_analyze` `type='ready'` → unblocked tasks
2. If ready tasks exist:
   a. `trellis_analyze` `type='critical_path'` → bottleneck chain
   b. Recommend task at intersection (ready AND on critical path)
   c. `trellis_task` `action='update'` `status='in_progress'` → claim it
   d. `trellis_context` `depth='agent'` → hand off implementation context
3. If NO ready tasks:
   a. `trellis_analyze` `type='plannable'` → draft tasks ready for planning
   b. `trellis_analyze` `type='critical_path'` → prioritize plannable tasks on bottleneck chain
   c. Tell user: "Nothing is ready to code yet — these tasks are ready to be planned."
   d. Follow "Plan a draft task" for the chosen task

### Dispatch to multiple agents
1. `trellis_analyze` `type='ready'` → all unblocked tasks (inherently parallelizable)
2. If ready tasks exist:
   a. Recommend N tasks for N agents, ranked by critical path
   b. Each agent claims: `trellis_task` `action='update'` `status='in_progress'`
   c. Each gets context: `trellis_context` `depth='agent'`
3. If NO ready tasks (or fewer ready than agents):
   a. `trellis_analyze` `type='plannable'` → draft tasks ready for planning
   b. Assign remaining agents to plan draft tasks in parallel
   c. Each: `trellis_context` `depth='planning'` → write plan → `trellis_task` `action='update'` `implementationPlan=...` `status='planned'`

### Implement a task (ALWAYS follow this sequence)
0. If task is `draft`, it MUST be planned first — follow "Plan a draft task" workflow before proceeding.
1. `trellis_task` `action='update'` `status='in_progress'` → claim it first (prevents double-assignment)
2. `trellis_context` `depth='agent'` → multi-hop deps, execution records, acceptance criteria
3. Do the implementation work
4. **Record what you built** — prepare these fields before marking done:
   - `executionRecord`: 3-5 sentences — what was built, approach, concrete details (function names, file paths, endpoints). No debugging stories.
   - `decisions`: one-liner per key technical choice (CHOICE + WHY)
   - `files`: every file created or modified
5. `trellis_task` `action='update'` `status='done'` `executionRecord='...'` `decisions=[...]` `files=[...]`
6. Run **Propagate Changes** on the completed task

**REQUIRED**: Steps 4-6 are NOT optional. Execution records feed downstream tasks via `trellis_context depth='agent'`. Skipping them breaks the context chain.

### Plan a draft task
1. `trellis_context` `depth='planning'` → spec + prerequisites + related work
2. Write the implementation plan:
   - **If plan mode was used**: read the plan file (e.g. `~/.claude/plans/*.md`) and use its full content
   - **Otherwise**: write a detailed plan — file paths, line numbers, specific changes, edge cases, verification steps
3. `trellis_task` `action='update'` `implementationPlan='<full plan content>'` `status='planned'`
   - Save the **complete, unabridged plan** — do not summarize

### Mark task done
1. `trellis_query` `type='search'` → find the task
2. If not already `in_progress`, set it first via `trellis_task` `action='update'` `status='in_progress'`
3. Collect execution details:
   - **User described what they did**: extract executionRecord, decisions, files from conversation
   - **User just said "done"**: ask what was built, key decisions, files touched
   - **Coding agent reported back**: summarize the agent's work into executionRecord
4. `trellis_task` `action='update'` with `status='done'`, `executionRecord`, `decisions`, `files` — **all three required**
5. Run **Propagate Changes** on the completed task
6. Report what was unlocked: `trellis_analyze type='ready'`

### Continue / Resume
1. `trellis_project` `action='list'` + `action='select'` → restore context
2. `trellis_query` `type='overview'` → big picture
3. `trellis_analyze` `type='ready'` → what's available
4. `trellis_analyze` `type='blocked'` → what's stuck
5. If no ready tasks: `trellis_analyze` `type='plannable'` → what can be planned
6. Summarize: progress, blockers, and concrete next-step recommendations

### Propagate Changes (run after every status change)
1. `trellis_query` `type='edges'` on the changed task → current relationships
2. `trellis_analyze` `type='downstream'` → tasks that depend on this one
3. For each downstream task, check:
   - Do edge notes need updating to reflect new decisions?
   - Are there NEW relationships revealed by this change?
   - Are there STALE relationships that no longer make sense?
   - Do downstream descriptions/criteria need updating?
4. Create/update/remove edges as needed

### Refine a task
1. `trellis_context` `depth='working'` → understand current state
2. Help improve description, acceptance criteria, decisions, dependencies
3. `trellis_task` `action='update'` → save changes

### Create a task
1. `trellis_task` `action='create'` with title (verb+noun), description, acceptanceCriteria, category, and tags
2. `trellis_edge` `action='create'` for any dependencies or relationships
3. Verify: `trellis_query` `type='edges'` on the new task — confirm edges look correct

### Delete a task
1. `trellis_task` `action='delete'` → preview mode (shows impact)
2. Show user the impact, wait for confirmation
3. `trellis_task` `action='delete'` `preview=false` → execute

### Update project
→ `trellis_project` `action='update'` with `title`, `description`, or `status` (brainstorming → decomposing → active → archived).

### Project status
→ `trellis_query` `type='overview'`

### Update dependencies
→ `trellis_query` `type='edges'` to review → `trellis_edge` to create/update/remove as needed

### When user mentions a task by name
1. `trellis_query` `type='search'` → find it (response includes `state` and `_hints`)
2. Follow the `_hints` to pick the correct context depth:
   - `plannable` → `depth='planning'`
   - `ready` → `depth='agent'`
   - `blocked` / `done` / `in_progress` / `draft` → `depth='working'`

### Review edge quality
1. `trellis_query` `type='list'` → all tasks
2. For tasks with many connections: `trellis_query` `type='edges'` → check each edge has a note and correct type
3. Fix missing notes via `trellis_edge` `action='update'` — notes propagate to downstream agent context
4. Remove stale edges where the relationship no longer holds

### Verify on correction
When the user corrects task info (wrong status, bad description, missing edge):
1. `trellis_query` `type='search'` → find the task
2. `trellis_context` `depth='working'` → see current state
3. Fix via `trellis_task` `action='update'` or `trellis_edge` create/update/remove
4. `trellis_query` `type='edges'` → confirm no stale edges

## Status Values

`draft` → `planned` → `in_progress` → `done`

## Edge Types

- `depends_on`: source needs target done first
- `relates_to`: informational link

## Edge Type Decision Criteria

Use `depends_on` when the source task **cannot start or complete** without the target's output:
- Source needs code/APIs/schema built by the target
- Source needs decisions or configuration defined in the target

Use `relates_to` when tasks share context but **neither blocks the other**:
- Tasks touch the same area of code but can be built independently
- One task's decisions are useful context but not required

**When in doubt**: removing the target makes source impossible → `depends_on`. Just harder → `relates_to`.
