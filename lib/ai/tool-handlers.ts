/**
 * Shared tool handlers for the 6 Trellis tools.
...
/** Params for trellis_project (handler covers list/create/update; MCP handles select separately). */
...
/**
 * Handle trellis_project actions (list/create/update).
  ...
  try {
    switch (p.action) {
      case "create": {
        ...
        createHints.push("No edges. Add dependencies with trellis_edge.");
        if (!p.category) {
          createHints.push("No category. Use trellis_project to see project categories, then set one with trellis_task action='update'.");
        }
        if (!p.acceptanceCriteria || p.acceptanceCriteria.length === 0) {
          createHints.push("No acceptance criteria. Add testable done conditions with trellis_task action='update'.");
        }
        return ok({ ...task, _hints: createHints });
      }
      case "update": {
        if (!p.taskId) return fail("taskId required for update. Use trellis_query type='search' to find it.");
        const changes: Record<string, unknown> = {};
        if (p.title !== undefined) changes.title = p.title;
        if (p.description !== undefined) changes.description = p.description;
        if (p.status !== undefined) changes.status = p.status;
        ...
        if (p.status === "in_progress") {
          updateHints.push("Run trellis_context depth='agent' to get implementation context before starting.");
        }
        if (p.status === "done") {
...
export async function handleEdge(p: EdgeParams): Promise<ToolResult> {
  try {
    switch (p.action) {
      ...
      case "update": {
        if (!p.edgeId)
          return fail("edgeId required for update. Use trellis_query type='edges' to find edge IDs.");
      ...
      case "remove": {
        ...
        return fail(
          ...
          "Use trellis_query type='edges' to find edge details.",
        );
      }
    }
  } catch (e) {
...
/**
 * Handle trellis_context actions.
...
/**
 * Handle trellis_analyze actions.
...
export async function handleAnalyze(p: AnalyzeParams): Promise<ToolResult> {
  try {
    switch (p.type) {
      ...
      case "downstream": {
        if (!p.taskId)
          return fail("taskId required for downstream analysis. Use trellis_query type='search' to find it.");
